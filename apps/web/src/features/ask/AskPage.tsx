...
import { apiFetch, PROJECT_ID, type AskResponse } from '../../api/client'
...
export function AskPage() {
  ...
  const ask = async (event?: FormEvent) => { event?.preventDefault(); setLoading(true); setDetails(false); try { setAnswer(await apiFetch<AskResponse>('/api/ask', { method: 'POST', body: JSON.stringify({ project_id: PROJECT_ID, question }) })) } finally { setLoading(false) } }
