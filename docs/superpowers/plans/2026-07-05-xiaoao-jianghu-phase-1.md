...
- [ ] **Step 2: 运行并确认 `/api/ontology` 返回 404**
...
- [ ] **Step 2: 运行并确认 `GraphImporter` 不存在**
...
- [ ] **Step 4: 实现 SQLite 项目仓储和 Neo4j 幂等导入**
...
- [ ] **Step 6: 启动 Neo4j，连续导入两次并验证计数不增加**
...
- [ ] **Step 2: 运行并确认测试因服务未编排而失败**

Run: `npm --prefix tests/e2e test`

Expected: 连接失败或缺少 webServer 配置。

- [ ] **Step 3: 配置 Playwright webServer 与 `make verify`**
